...
struct SubjectRing: View {
    ...
    struct Lapping: Equatable {
        ...
        /// What the turn on top casts onto the turn beneath it. Shared with
        /// the goal ring, which laps in the same way in the same place.
            ...
            ForEach(laps) { lap in
                ...
                if lap.id == top, let stop = lap.stops.last {
                    capArc(lap.tailBand, color: color(stop.colorIndex, lap: lap.id, top: top))
                        .ringShadow(lapping.shadow, cast: top > 0)
                }
