...
| START-04 | P1 | device | Committing suppresses exactly one click, and no more ([B-12](../bug-triage.md#b-12)) | Fresh launch | Turn the crown a third of a step so the reading stays 0. Tap Start. End the session. Turn the crown one step. | A click on that step | — |
| START-05 | P1 | device | The crown still works after a sheet ([B-12b](../bug-triage.md#b-12)) | Fresh launch | Open the subject picker, dismiss it, turn the crown | The duration changes | — |
...
| START-11 | P3 | device, VoiceOver | The ring's progress is announced ([B-15](../bug-triage.md#b-15)) | 1h of 2h studied | Swipe to the ring | Expected to FAIL: only "Today" and the total are spoken | — |
...
| RUN-09 | P1 | device, largest text size | The bottom bar fits at the largest text size ([B-14](../bug-triage.md#b-14)) | Any session, text size at max | Tap End once | Expected to FAIL: "END?" and both controls overflow the bar | — |
...
| RUN-11 | P1 | device | ⚠️ Run RUN-10 only after [B-01](../bug-triage.md#b-01) is fixed | — | — | — | blocked |
| RUN-12 | P3 | device | 44pt plus a `+` does not overrun the ring ([B-16](../bug-triage.md#b-16)) | 1m session on a 41mm watch | Let it run past zero | `+00:07` sits inside the arc | — |
| RUN-13 | P1 | device | The deadline notification is delivered while the app holds the foreground ([B-18](../bug-triage.md#b-18)) | 1m session, notifications allowed, wrist down | Wait past the deadline | A wrist tap within a few seconds of the deadline | — |
...
| ID | P | Needs | Claim | Setup | Steps | Expected | Result |
| --- | --- | --- | --- | --- | --- | --- | --- |
| DONE-01 | P1 | device | A held session is wrongly called Complete ([B-02](../bug-triage.md#b-02)) | 25m session | Study 5m, hold 30m, resume, study 1m, end | Expected to FAIL: the summary says "Complete" and plays the success haptic for 6m of 25m | — |
...
| DONE-06 | P3 | device, VoiceOver | An armed Discard announces its state ([B-15](../bug-triage.md#b-15)) | Any kept session | VoiceOver onto Discard, activate | Expected to FAIL: the label changes with no announcement | — |
...
| PICK-04 | P3 | device | Rows meet the 44pt floor ([B-13](../bug-triage.md#b-13)) | Any | Measure a row | Expected to FAIL: 40pt | — |
...
| SET-05 | P3 | device | Swatch cells meet the 44pt floor ([B-13](../bug-triage.md#b-13)) | Any subject | Measure a cell | Expected to FAIL: 34pt | — |
| SET-06 | P1 | device | Delete asks for confirmation ([B-21](../bug-triage.md#b-21)) | Any subject | Tap Delete | Expected to FAIL: it deletes and pops immediately | — |
...
| SET-08 | P2 | device | Duplicate names are allowed ([B-20](../bug-triage.md#b-20)) | Any | Add "Maths" twice | Expected: both are created, indistinguishable but for colour | — |
| SET-09 | P2 | device | The eleventh subject gets a fresh colour ([B-22](../bug-triage.md#b-22)) | Ten subjects | Add an eleventh | Expected to FAIL: it takes the first subject's colour | — |

## Metrics — `MET`

| ID | P | Needs | Claim | Setup | Steps | Expected | Result |
| --- | --- | --- | --- | --- | --- | --- | --- |
| MET-01 | P1 | device | "Today" advances while a session runs ([B-09](../bug-triage.md#b-09)) | Session started from the Action Button | Open Metrics from Start, wait two minutes | Expected to FAIL: the number does not move | — |
...
| MET-05 | P3 | device, VoiceOver | Weekday bars announce the day ([B-15](../bug-triage.md#b-15)) | Any week | VoiceOver onto a bar | Expected to FAIL: a single letter, not the weekday | — |
| MET-06 | P3 | device, VoiceOver | The heatmap announces its data ([B-15](../bug-triage.md#b-15)) | Any history | VoiceOver onto the heatmap | Expected to FAIL: "Last twelve weeks" and nothing else | — |
...
| PAIR-03 | P2 | device, network | Claiming the code changes nothing on the watch ([B-24](../bug-triage.md#b-24)) | Online | Show a code, claim it on the web | Expected: the watch never says anything | — |
| PAIR-04 | P3 | device, VoiceOver | The code is spelled in the case shown ([B-23](../bug-triage.md#b-23)) | Online, a lowercase code from the server | VoiceOver onto the code | Expected to FAIL if codes are ever lowercase | — |
