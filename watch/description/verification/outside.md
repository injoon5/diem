...
| CARD-05 | P1 | device, patience | The End button works after the extension has been rebuilt ([B-03](../bug-triage.md#b-03)) | Start a session in the app. Leave the watch untouched long enough for the extension to be torn down (unknown; try an hour). | Tap End on the card | Expected to FAIL: "Nothing is running." and the session keeps running | — |
| CARD-06 | P1 | device | The app notices a session ended from the card ([B-03](../bug-triage.md#b-03)) | Session running, app backgrounded but recently used | End from the card, then reopen the app | Expected to FAIL: the Running screen is still up, clock stopped, labelled "Paused" | — |
| CARD-07 | P1 | device | Resuming after CARD-06 does not revive the session ([B-03](../bug-triage.md#b-03)) | Following CARD-06 | Tap Resume | Expected to FAIL: a new interval is spliced into an ended session | — |
| CARD-08 | P2 | device | Ending from the card with the app on screen fires one haptic ([B-11](../bug-triage.md#b-11)) | Session running, app on screen, card pulled over it | End from the card | Expected to FAIL: two haptics — success then stop | — |
...
| COMP-04 | P1 | device, overnight | The total resets at the 4am boundary ([B-07](../bug-triage.md#b-07)) | 2h studied, watch left alone from 03:45 | Look at the complication at 04:05 | Expected to FAIL: yesterday's two hours and a closed ring | — |
...
| COMP-08 | P3 | device, VoiceOver | The circular family speaks a usable value ([doc](../outside/complications.md#cross-cutting)) | 90m studied | VoiceOver onto it | Expected: "1.5h" — a product call, not a failure | — |
...
| INT-02 | P1 | device | Starting silently ends a running session ([B-10](../bug-triage.md#b-10)) | A session running for 20m | Press the Action Button | Expected to FAIL: the first session ends with no summary and no mention; the dialog says only "Studying." | — |
